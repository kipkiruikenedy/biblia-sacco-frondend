<template>
  <div class="container mx-auto mt-8">
    <h1 class="text-3xl font-semibold mb-4">Explore Our Loan Services</h1>
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      <div v-for="loan in loans" :key="loan.id">
        <div @click="toggleDescription(loan.id)" class="cursor-pointer bg-white rounded-lg shadow-md p-4 transition-transform transform hover:scale-105">
          <div class="flex items-center justify-between">
            <h2 class="text-xl font-semibold">{{ loan.name }}</h2>
            <i :class="loan.open ? 'fas fa-chevron-up' : 'fas fa-chevron-down'"></i>
          </div>
          <div v-if="loan.open" class="mt-4 text-gray-600">{{ loan.description }}</div>
        </div>
      </div>
    </div>
  </div>
...
const loans = ref([
  ...
  {
    id: 2,
    name: 'Asset Finance Loan / Product Loan',
    description: `
...
Applicable to members with deposits of at least 15% of the loan applied.
  ...
  },
  {
    ...
    description:`Members to apply for the loan under the normal terms.
...
Maximum repayment period shall be 60 months.`,
  ...
  },
  {
    id: 4,
    name: 'Daraja Loan',
    description: `Maximum amount shall be Kshs 400,000.00 or an amount set by the BOD.
...
Maximum repayment period shall be 24 months.`,
  ...
  },
  {
    id: 5,
    name: 'M-Biblia Loan',
    description: ` This is a mobile banking facility that allows members the opportunity to access loans via their phones.
...
Cheque is payable directly to the insurance company.`,
  ...
  },
  {
    id: 7,
    name: 'Elimu Loan',
    description:`Maximum amount shall be Kshs. 500,000.00 or an amount set by BOD.
Interest shall be charged at 12% p.a on reducing balance and may be varied from time to time by the BOD.
Maximum repayment period shall be 24 months.
Application for school fees loan should be submitted together with a signed and stamped fee structure.`,
  ...
  },
  {
    id: 8,
    name: 'Makao Poa Loan',
    description: `This product is designed to help members in their investment and development plans e.g.acquire a plot, renovate, finish or even construct commercial, rental and residential properties.
...
Applicable to members with deposits of at least 15% of the loan applied`,
  ...
  },
  {
    id: 9,
    name: 'Upesi Loan',
    description:`Maximum amount shall be Kshs. 300,000.00, or an amount determined by the BOD from time to time.
...
const toggleDescription = (loanId) => {
