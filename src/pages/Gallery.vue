  ...
  <div class="min-h-screen bg-gray-100 p-4">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      ...
      <div v-for="image in images" :key="image.id">
        <img :src="getImageUrl(image.filename)" :alt="image.filename" class="w-full h-auto rounded-md shadow-md mb-4">
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    async fetchImages() {
      try {
        // Replace '/api/gallery/images' with the actual endpoint for fetching gallery images
